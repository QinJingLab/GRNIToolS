...
parser.add_argument('-tf2_network_path', required=False,default='/mnt/sdb/chenzhb/GRNDL_master/example/data/ecoli/Deeplearning data/train_val_test/TF_test.txt')
...
test3_data = torch.from_numpy(validation_data)
...
test3_data=test3_data.to(device)
...
score5 = model(data_feature, adj, test3_data)
if args.flag:
...
AUC5, AUPR5, AUPR_norm5= Evaluation(y_pred=score5, y_true=test3_data[:, -1],flag=args.flag)
end_time=time.time()
...
print('train_AUC:{}'.format(AUC1),
    ...
    '\ntest3_AUC:{}'.format(AUC5),
     'test3_AUPRC:{}'.format(AUPR5))
...
with open(f'{output_path}/auc_result.txt', 'w', newline='', encoding='utf-8') as file:
    ...
    file.write('TF2_auc={:.5f}\tTF1_aupr={:.5f}\n'.format(AUC3, AUPR3))
    file.write('TF3_auc={:.5f}\tTF2_aupr={:.5f}\n'.format(AUC4, AUPR4))
    file.write('TF1_auc={:.5f}\tTF3_aupr={:.5f}\n'.format(AUC5, AUPR5))
