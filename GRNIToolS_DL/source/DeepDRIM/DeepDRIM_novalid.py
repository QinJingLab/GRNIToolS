...
parser.add_argument('-epoch', type=int, required=False, default="100", help="The epoch used in model")
...
class SaveModelOnFirstAccuracy(Callback):
    def __init__(self, save_dir, accuracy_thresholds):
        super(SaveModelOnFirstAccuracy, self).__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        timefile=self.save_dir+'/model_time.txt'
        if accuracy >=0.99:
            self.model.stop_training=True
        
        if self.accuracy_thresholds and accuracy >= self.accuracy_thresholds[0] and self.accuracy_thresholds[0] not in self.saved_models:
             filepath = self.save_dir + '/model_acc_{}.h5'.format(self.accuracy_thresholds[0])
             print("self.accuracy_thresholds[0]="+str(self.accuracy_thresholds[0]))
             print("Saved model with accuracy"+str(self.accuracy_thresholds[0]))
             self.model.save(filepath)
             self.saved_models.append(self.accuracy_thresholds[0])
             end_time=time.time()
             execution_time=(end_time - start_time)/60
             #timefile=self.save_dir+'/'+str(epoch)+str(self.accuracy_thresholds[0])+'_time.txt'
             with open(timefile,'a',newline='',encoding='utf-8')as file:
                  file.write('epoch\t'+str(epoch)+'\ttime\t'+str(execution_time)+'min\n')

             self.accuracy_thresholds.pop(0)
...
class direct_model1_squarematrix:
    def __init__(self, num_batches=5, output_dir=None, data_path=None, predict_output_dir=None):
        ...
        # num_predictions =args.batch_size
        self.batch_size = args.batch_size  # mini batch for training
        # num_classes = 3   # ### categories of labels
        self.epochs = args.epoch  # ### iterations of trainning, with GPU 1080, 200 for KEGG and Reactome, depends on specific tasks for GTRD, we actually selected
    ...
    def construct_model(self, x_train):
        ...
        if self.num_classes < 2:
            ...
            lr_schedule = ExponentialDecay(
                ...
                decay_steps=1000,  # 每多少步衰减一次
                decay_rate=0.96     # 衰减率
            )
            sgd = SGD(learning_rate=lr_schedule, momentum=0.9, nesterov=True)
            # sgd = SGD(learning_rate=args.lr, decay=1e-6, momentum=0.9, nesterov=True)
            model.compile(optimizer=sgd, loss='binary_crossentropy', metrics=['accuracy'])
        #stop the early_stopping method
        early_stopping = keras.callbacks.EarlyStopping(monitor='accuracy', patience=2000, verbose=0, mode='auto')
        checkpoint1 = ModelCheckpoint(filepath=self.save_dir + '/weights.{epoch:02d}-{val_loss:.2f}.keras',
                                      monitor='val_loss',
                                      verbose=1, save_best_only=False, save_weights_only=False, mode='auto', save_freq=5)
        checkpoint2 = ModelCheckpoint(filepath=self.save_dir + '/weights.keras', monitor='accuracy', verbose=1,
                                      save_best_only=True, mode='auto', save_freq=5)
        accuracy_thresholds = [0.7,0.75, 0.8,0.85, 0.9, 0.95, 0.99]
        save_model_callback = SaveModelOnFirstAccuracy(save_dir=self.save_dir+'/different_acc', accuracy_thresholds=accuracy_thresholds)

        callbacks_list = [checkpoint2, early_stopping,save_model_callback]
        self.model = model
  ...
  #      if self.num_classes == 2:  ## here we only focus on three category tasks
   ...
   #         auc = np.trapz(tpr, fpr)
    ...
    #        file_handle=open(save_dir+'/AUC.txt',mode='a')
    #        file_handle.write('AUC:'+str(auc))
     #       file_handle.close()
     #       plt.savefig(save_dir + '/end_3labels.pdf')
        ...
        if self.num_classes == 3:
            ...
            # Print AUC
            auc = np.trapz(tpr, fpr)
            file_handle=open(save_dir+'/rocAUC.txt',mode='a')
            file_handle.write('AUC:'+str(auc))
            file_handle.close()
            print('AUC:', auc)
            plt.ylim([0, 1])
    ...
    def train_and_test_model(self,num_of_pair_ratio=1):
        for test_indel in range (self.num_batches):                                      #  for GTRD leave-one-TF-out CV
            ...
            if not self.data_augmentation:
                ...
                history = model.fit(x_train_list, y_train,batch_size=self.batch_size,epochs=self.epochs,validation_split=0,
    ...
    def predict_use_model(self, weight_path=""):
        ...
        test_indel = []  # the name of data. (0_xdata.npy, 0_ydata.npy, 0_zdata.npy)
        ...
        for test_indel in range(self.num_batches):                                      #  for GTRD leave-one-TF-out CV
            ...
            all_predict=np.append(all_predict,y_predict)
        ...
        np.savetxt(self.predict_output_dir+'predict.csv',all_predict,delimiter=",")
        if args.label_path is not None:
            label=pd.read_csv(args.label_path,usecols=[2],sep='\t',header=None)
            auc=roc_auc_score(label,all_predict)
            aupr=average_precision_score(label,all_predict)
            with open(self.predict_output_dir+'auc_result.txt','w')as file:
                file.write(f'AUC:{auc}\n')
                file.write(f'AUPR:{aupr}\n')
            print(f'AUC: {auc:.5f}',f'AUPR: {aupr:.5f}')
    def load_indel_lists_from_file(self, cross_validation_fold_divide_file):
        ...
        indel_list1=cross_fold[1]+cross_fold[2]
        indel_list2=[]
...
end_time=time.time()
execution_time=(end_time - start_time)/60
timefile=args.output_dir+'time.txt'
with open(timefile,'w',newline='',encoding='utf-8')as file:
    file.write(str(execution_time)+'min')
