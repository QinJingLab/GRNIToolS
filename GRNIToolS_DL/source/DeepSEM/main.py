...
############evaluation###############
import subprocess
def eva(grn_file, hgs_file, method, dataset, output_dir):

    command = ['python', './source/DeepSEM/src/evaluate.py', '--hgs', hgs_file, '--grn', grn_file, '--method', method, '--dataset', dataset, '--output', output_dir]
    print(' '.join(command)) 
    result = subprocess.run(command, check=True, capture_output=True, text=True)
    with open(f'{output_dir}/{dataset}_auc.txt') as f:
        res = f.readlines()[1]
    return res

net_file = opt.net_file

r1 = eva(f'{opt.save_name}/GRN_inference_result.tsv', net_file, 'DeepSEM', 'all',  f'{opt.save_name}')

res = 'method\tdataset\tauroc\taupr\n'
res += r1 + r1 + r1
with open(f'{opt.save_name}/roc.txt', 'w') as f:
    f.write(res)
