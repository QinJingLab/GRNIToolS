...
def gen_geneNameDict(datasetPath, geneExpData, refNetwork):
    ...
    all_genes = pd.concat([geneExpData,tf, target])
    all_genes = all_genes.drop_duplicates()

    geneName = pd.DataFrame({'geneName': all_genes,
                             'index': range(all_genes.shape[0])})
...
def gen_dataSplit(datasetPath, TF, Target, Edge, k):
    ...
    # append(negEdge.iloc[int(negEdgeNum*(i/k)):int(negEdgeNum*((i+1)/k))])
        df1 = posEdge.iloc[int(posEdgeNum*(i/k)):int(posEdgeNum*((i+1)/k))]
        df2 = negEdge.iloc[int(negEdgeNum*(i/k)):int(negEdgeNum*((i+1)/k))]
        split_df = pd.concat([df1, df2])
