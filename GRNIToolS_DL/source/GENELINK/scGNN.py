...
class AttentionLayer(nn.Module):
    ...
    def forward(self,x,adj):
        ...
        # 只计算存在边的位置
        edge_e = e[row, col]  # 提取邻接矩阵对应位置的注意力分数
        edge_e = F.leaky_relu(edge_e, negative_slope=self.alpha)
        
        # 稀疏softmax实现
        max_per_row = torch.zeros_like(e[:, 0]).scatter_reduce(
            0, row, edge_e, reduce='amax', include_self=False)
        edge_e = (edge_e - max_per_row[row]).exp()
        sum_per_row = torch.zeros_like(e[:, 0]).scatter_add(0, row, edge_e)
        attention_values = edge_e / (sum_per_row[row] + 1e-8)
        ...
        # 稀疏矩阵乘法
        h_pass = torch.sparse.mm(attention, h)
