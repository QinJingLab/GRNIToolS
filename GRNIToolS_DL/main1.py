...
parser.add_argument('-expression', required=False, default="data/mESC-PC/train_val_test/mESC-PC-expression.csv", help="Expression data path")
parser.add_argument('-output_dir', required=False, default="data/mESC-PC/train_val_test/input/", help="Indicate the path for output.")
parser.add_argument('-data_path', required=False, default="data/mESC-PC/train_val_test/train.txt", help="The path that includes train data")
#parser.add_argument('-test_data_path', required=False, default="data/mESC-PC/train_val_test/test.txt", help="The path that includes test data")
parser.add_argument('-dataset', required=False, default="mESC-PC-expression.csv", help="The name of dataset")
...
def get_origin_expression_data(gene_expression_path):
    ...
    expression_reader = [[cell.lower() for cell in row] for row in csv.reader(f_expression)]
...
##Read the extension to deal with csv file or txt file
    extension=gene_pair_list_path.split(".")[-1]
    label_list = []
    pair_list = []
    total_matrix = []
    num_tf = 0
...
##Deal with csv file
    if extension == "csv":
        print("csv file")
        for single_pair in list(csv.reader(f_genePairList)):
            single_pair=lower(single_pair)
            if single_pair[2] == '1':
...
##use our file as model_evaluator, so generate new data same as origin data
...
##Deal with txt file
    if extension == "txt":
        print("txt file")
        with open(gene_pair_list_path, 'r') as files:
            for line in files.readlines():
                single_pair = line.strip().lower().split('\t')
                if single_pair[2] == '1':
                    if single_pair[0] not in gold_pair_record:
                        gold_pair_record[single_pair[0]] = [single_pair[1]]
                    else:
...
##use our file as model_evaluator, so generate new data same as origin data
    ...
    np.save(resultPath + 'matrix2.npy', total_matrix)
    np.save(resultPath + 'label2.npy', label_list)
    np.save(resultPath + 'gene_pair2.npy', pair_list)
...
def loadDatatest(dataset,gene_pair_list_path,gene_expression_path,resultPath):
...
#known_network = "random_train.csv"
gene_pair_list_path = args.data_path
gene_expression_path =args.expression
#resultPath = "data/HGSData/input/known/"
resultPath=output
...
timefile=output+'time.txt'
print("running time:", execution_time,"min")
