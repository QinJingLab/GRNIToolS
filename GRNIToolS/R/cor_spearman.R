...
#' @description Default parameters for spearman algorithm.
...
#' grn <- cor_spearman(datexpr,gene)
#'
cor_spearman <- function(datexpr,gene,TF = NULL,file = NULL){
  datexpr <- t(datexpr)
  res <- Hmisc::rcorr(as.matrix(datexpr),type = 'spearman')
  exprmat <- res$r
  exprp <- res$P
