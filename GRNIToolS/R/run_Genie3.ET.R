#' GENIE3.ET
#' @description Default parameters for GENIE3 algorithm (Extra-Trees)
...
#'
#' @details GENIE3 Infers a gene regulatory network (in the form of a weighted adjacency matrix) from expression data, using ensembles of regression trees(Extra-Trees).
#'
#' @return Matrix returns a matrix which is the weighted adjacency matrix of the network inferred by GENIE3 algorithm (Extra-Trees).
#' @export
...
#' TF <- gene[1:20]
#' #grn <- run_GENIE3.ET(datexpr,gene,TF)
run_GENIE3.ET <- function(datexpr,gene,TF = NULL,file = NULL,cl.cores = 2){
  if (is.null(TF)) {
    TF <- gene
  }
  ...
  weightMatrix <- GENIE3::GENIE3(exprMatrix = datexpr,regulators = TF,nCores = cl.cores,verbose = F,treeMethod = 'ET')
